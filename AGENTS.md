...
This repository builds a Simplified Chinese language pack and patching tools for Antigravity IDE plus Copilot, Gemini, Kiro, and Qoder CLIs. `extension.js` is the VS Code/Antigravity extension entry point. Translation assets live in `translations/`: `extensions/` holds official i18n JSON files, while `patches/` holds replacement tables grouped by product, such as `ide/antigravity` and `cli/qoder`. Python patching and extraction tools live in `scripts/`, split into `ide/`, `cli/`, and shared helpers in `scripts/shared/`. Tests are under `tests/`, with a few legacy root-level test files. `datafiles/` stores source comparison data used to generate replacement tables.
